...
$xamlFile = Join-Path $global:appBaseDir "TextSearchWPF.xaml"
$themesFile = Join-Path $global:appBaseDir "TextSearchWPF_Themes.json"
$engineFile = Join-Path $global:appBaseDir "TextSearchWPF_Engine.cs"
$templateFile = Join-Path $global:appBaseDir "TextSearchWPF_Template.html"
...
$appDataDir = Join-Path $env:LOCALAPPDATA "TextSearchWPF_Cache"
...
$configFile = "$global:appBaseDir\TextSearchWPF.json"
...
function Show-Reader($directFilePath = $null) {
    ...
    $rForm = New-Object System.Windows.Forms.Form -Property @{ Text="Modern Reader - $([System.IO.Path]::GetFileName($p))"; Size=New-Object System.Drawing.Size([math]::Max(1020,$script:rW),$script:rH); MinimumSize="1020,550"; FormBorderStyle='Sizable'; MaximizeBox=$true; AutoScaleMode='Dpi'; KeyPreview=$true }
...
'@
    ...
    $btnReset.Add_Click({ 
        ...
        if ($frm -and -not $frm.Tag.initializing) { 
            $pSet = $frm.Controls["pnlSettings"]
            $idx = $pSet.Controls["cmbTheme"].SelectedIndex
            $tFile = Join-Path $global:appBaseDir "TextSearchWPF_Themes.json"
            if (Test-Path $tFile) {
