...
# --- THEME COLOUR HELPERS (used to tint the theme picker menu) ---
function Global:Test-HexIsLight($hex) {
    try {
        $c = [System.Drawing.ColorTranslator]::FromHtml($hex)
        return ((0.299*$c.R + 0.587*$c.G + 0.114*$c.B) / 255.0) -gt 0.5
    } catch { return $false }
}

function Global:Get-HexShifted($hex, $factor) {
    try {
        $c = [System.Drawing.ColorTranslator]::FromHtml($hex)
        $r = [Math]::Min(255, [Math]::Max(0, [int]($c.R + (255 * $factor))))
        $g = [Math]::Min(255, [Math]::Max(0, [int]($c.G + (255 * $factor))))
        $b = [Math]::Min(255, [Math]::Max(0, [int]($c.B + (255 * $factor))))
        return "#{0:X2}{1:X2}{2:X2}" -f $r, $g, $b
    } catch { return $hex }
}

# A theme's FN is a CSS stack ("'Merriweather', 'Georgia', serif"), but the Font
# combo lists bare installed family names - a direct match never succeeds. Walk the
# stack and return the first family actually installed, or $null if none are.
...
# Turn a CSS font stack ("'Merriweather', 'Georgia', serif") into a WPF
# FontFamily fallback list ("Merriweather, Georgia, Segoe UI").
function Global:ConvertTo-WpfFontFamily($cssStack) {
    try {
        $generic = @('serif','sans-serif','monospace','cursive','fantasy','system-ui','ui-serif','ui-sans-serif','ui-monospace')
        $names = @()
        foreach ($part in ($cssStack -split ',')) {
            $n = $part.Trim().Trim("'").Trim('"').Trim()
            if ($n -and ($generic -notcontains $n.ToLower()) -and ($n -notmatch '[<>&]')) { $names += $n }
        }
        if ($names -notcontains 'Segoe UI') { $names += 'Segoe UI' }
        return ($names -join ', ')
...
# -Encoding UTF8 is required: the themes file has no BOM, and Windows PowerShell
# would otherwise read it as ANSI and mangle accented names (e.g. "Rose Pine Dawn").
...
function Show-Reader($directFilePath = $null) {
    if (-not $global:webViewReady) { 
        [System.Windows.Forms.MessageBox]::Show("WebView2 wrapper DLLs could not be loaded.`n`nError: $($global:webViewError)`n`nPlease close the app, delete the DLLs in the folder, and restart.", "Missing Dependencies", 0, 'Error') | Out-Null
        return 
    }
    ...
    $btnTheme = New-Ctrl Button @{Name="btnThemePicker"; Location='60,11'; Size='130,25'; Text=$script:themes[$script:activeTheme].Name} $pnlSettings
    $cmbTheme = New-Ctrl ComboBox @{Name="cmbTheme"; Location='0,0'; Size='0,0'; Visible=$false; DropDownStyle='DropDownList'} $pnlSettings
    $script:themes | ForEach-Object { [void]$cmbTheme.Items.Add($_.Name) }
    $cmbTheme.SelectedIndex = $script:activeTheme
    
    $btnTheme.Add_Click({
      try {
        # NOTE: variables local to Show-Reader (e.g. $btnTheme) are NOT visible here.
        # An event scriptblock runs after the function has returned, so resolve
        # everything from $this / OpenForms, exactly like the other handlers do.
        $btn = $null; try { $btn = $this } catch {}
        $frm = $null; if ($btn) { try { $frm = $btn.FindForm() } catch {} }
        if (-not $frm) { foreach ($f in [System.Windows.Forms.Application]::OpenForms) { if ($f.Controls["pnlSettings"]) { $frm = $f; break } } }
        if (-not $frm) { return }
        $pSet = $frm.Controls["pnlSettings"]
        if (-not $pSet) { return }
        $cmbSel = $pSet.Controls["cmbTheme"]
        $lblBtn = $pSet.Controls["btnThemePicker"]
        if (-not $cmbSel -or -not $lblBtn) { return }

        # --- Tint the picker with the currently active theme (bg / text / highlight / font) ---
        $mBg = '#1A1A1A'; $mFg = '#E0E0E0'; $mBorder = '#333333'
        $mHover = '#333333'; $mHoverFg = '#FFFFFF'; $mMuted = '#888888'; $mFont = 'Segoe UI'
        try {
            $curIdx = $cmbSel.SelectedIndex
            if ($curIdx -ge 0 -and $curIdx -lt $script:themes.Count) {
                $ct = $script:themes[$curIdx]
                $isLight = Test-HexIsLight $ct.Bg
                # Menu sits slightly above the page: lift on dark themes, deepen on light ones
                $mBg      = Get-HexShifted $ct.Bg $(if ($isLight) { -0.06 } else { 0.08 })
                $mFg      = $ct.Tx
                $mBorder  = Get-HexShifted $ct.Bg $(if ($isLight) { -0.15 } else { 0.15 })
                $mHover   = $ct.Hi
                $mHoverFg = $(if (Test-HexIsLight $ct.Hi) { '#000000' } else { '#FFFFFF' })
                $mMuted   = Get-HexShifted $ct.Tx $(if ($isLight) { 0.25 } else { -0.25 })
                $mFont    = ConvertTo-WpfFontFamily $ct.FN
            }
        } catch {}

        $cmXaml = @"
<ContextMenu xmlns='http://schemas.microsoft.com/winfx/2006/xaml/presentation' xmlns:x='http://schemas.microsoft.com/winfx/2006/xaml' Background='$mBg' Foreground='$mFg' BorderBrush='$mBorder' BorderThickness='1' Padding='4' FontFamily='$mFont'>
  <!-- Own template: the stock one draws a system-coloured icon gutter down the left. -->
  <ContextMenu.Template>
    <ControlTemplate TargetType='ContextMenu'>
      <Border Background='{TemplateBinding Background}' BorderBrush='{TemplateBinding BorderBrush}' BorderThickness='{TemplateBinding BorderThickness}' Padding='{TemplateBinding Padding}' CornerRadius='4' SnapsToDevicePixels='True'>
        <ItemsPresenter/>
      </Border>
    </ControlTemplate>
  </ContextMenu.Template>
  <ContextMenu.Resources>
    <!-- The columns live inside one host MenuItem. Without a bare template the host
         would inherit the item style below and paint its highlight across the whole menu. -->
    <ControlTemplate x:Key='PlainHost' TargetType='MenuItem'>
        <ContentPresenter ContentSource='Header'/>
    </ControlTemplate>
    <Style TargetType='MenuItem'>
        <Setter Property='Foreground' Value='$mFg'/>
            ...
            <Setter.Value>
                <ControlTemplate TargetType='MenuItem'>
                    <Border Name='Border' Background='Transparent' CornerRadius='2'>
                        <Grid>
                            <Path x:Name='CheckMark' Visibility='Collapsed' Data='M 0,5 L 3,8 L 9,0' Stroke='$mFg' StrokeThickness='2' Margin='10,0,0,0' VerticalAlignment='Center' HorizontalAlignment='Left'/>
                            <ContentPresenter ContentSource='Header' Margin='{TemplateBinding Padding}' RecognizesAccessKey='True'/>
                        </Grid>
                    </Border>
                    <ControlTemplate.Triggers>
                        <Trigger Property='IsHighlighted' Value='True'>
                            <Setter TargetName='Border' Property='Background' Value='$mHover'/>
                            <Setter Property='Foreground' Value='$mHoverFg'/>
                        </Trigger>
  ...
  <MenuItem StaysOpenOnClick='True' Template='{StaticResource PlainHost}'>
    <MenuItem.Header>
      <StackPanel Orientation='Horizontal'>
        <StackPanel x:Name='colDark' Margin='0,0,16,0'><TextBlock Text='Dark Themes' Foreground='$mMuted' FontWeight='SemiBold' Margin='12,4,8,8' FontSize='11'/></StackPanel>
        <StackPanel x:Name='colLight' Margin='0,0,16,0'><TextBlock Text='Light Themes' Foreground='$mMuted' FontWeight='SemiBold' Margin='12,4,8,8' FontSize='11'/></StackPanel>
        <StackPanel x:Name='colMono'><TextBlock Text='Mono Themes' Foreground='$mMuted' FontWeight='SemiBold' Margin='12,4,8,8' FontSize='11'/></StackPanel>
      </StackPanel>
    </MenuItem.Header>
  </MenuItem>
</ContextMenu>
"@
        # Keep a script-scope reference so the popup is not collected while open
        $script:themeMenu = [Windows.Markup.XamlReader]::Parse($cmXaml)
        $cm = $script:themeMenu
        $colDark = $cm.FindName("colDark")
        $colLight = $cm.FindName("colLight")
        $colMono = $cm.FindName("colMono")
        if (-not $colDark -or -not $colLight -or -not $colMono) { return }

        $selIdx = $cmbSel.SelectedIndex
        $darkList = @(); $lightList = @(); $monoList = @()
        for ($i = 0; $i -lt $script:themes.Count; $i++) {
            $t = $script:themes[$i]
            $mi = New-Object System.Windows.Controls.MenuItem
            $mi.Header = $t.Name

            # Captured by value via GetNewClosure() so each item keeps its own index
            $miIdx = $i; $miName = $t.Name; $miMenu = $cm; $miCmb = $cmbSel; $miBtn = $lblBtn
            $mi.Add_Click({
                try {
                    $miMenu.IsOpen = $false
                    $miCmb.SelectedIndex = $miIdx
                    $miBtn.Text = $miName
                } catch {}
            }.GetNewClosure())

            if ($i -eq $selIdx) { $mi.IsChecked = $true }

            if ($t.FN -match "mono") { $monoList += $mi }
            else {
                try {
                    $c = [System.Drawing.ColorTranslator]::FromHtml($t.Bg)
                    $brightness = (0.299*$c.R + 0.587*$c.G + 0.114*$c.B)
                    if ($brightness -gt 128) { $lightList += $mi } else { $darkList += $mi }
                } catch { $darkList += $mi }
            }
        }

        $darkList | Sort-Object Header | ForEach-Object { [void]$colDark.Children.Add($_) }
        $lightList | Sort-Object Header | ForEach-Object { [void]$colLight.Children.Add($_) }
        $monoList | Sort-Object Header | ForEach-Object { [void]$colMono.Children.Add($_) }

        $cm.IsOpen = $true
      } catch {
        [System.Windows.Forms.MessageBox]::Show("Theme menu error: $($_.Exception.Message)", "ZenSeek", 'OK', 'Warning') | Out-Null
      }
    })
    ...
    $btnSettings.Add_Click({
        $frm = $null; try { $frm = $this.FindForm() } catch {}; if (-not $frm) { foreach ($f in [System.Windows.Forms.Application]::OpenForms) { if ($f.Controls["pnlSettings"]) { $frm = $f; break } } }
        if ($frm) {
    ...
    $updateViewer = {
        ...
        $activeMatchLine = -1
        if ($frm.Tag.matchIndices.Count -gt 0) { $activeMatchLine = $frm.Tag.matchIndices[$frm.Tag.index] }
        $targetLine = 0
        ...
        $isMatchJS = "true"
        if ($isRestoringPosition) { $isMatchJS = "false" }
    ...
    $btnPrevMatch.Add_Click({ $frm = $null; try { $frm = $this.FindForm() } catch {}; if (-not $frm) { foreach ($f in [System.Windows.Forms.Application]::OpenForms) { if ($f.Controls["pnlSettings"]) { $frm = $f; break } } }; if($frm){$frm.Tag.navMatch.Invoke($frm, -1)} })
    $btnNextMatch.Add_Click({ $frm = $null; try { $frm = $this.FindForm() } catch {}; if (-not $frm) { foreach ($f in [System.Windows.Forms.Application]::OpenForms) { if ($f.Controls["pnlSettings"]) { $frm = $f; break } } }; if($frm){$frm.Tag.navMatch.Invoke($frm, 1)} })
...
'@
    ...
    $numContext.Add_ValueChanged({ $frm = $null; try { $frm = $this.FindForm() } catch {}; if (-not $frm) { foreach ($f in [System.Windows.Forms.Application]::OpenForms) { if ($f.Controls["pnlSettings"]) { $frm = $f; break } } }; if($frm -and -not $frm.Tag.initializing){$frm.Tag.updateViewer.Invoke($frm)} })
    
    $btnOpenFile.Add_Click({ $frm = $null; try { $frm = $this.FindForm() } catch {}; if (-not $frm) { foreach ($f in [System.Windows.Forms.Application]::OpenForms) { if ($f.Controls["pnlSettings"]) { $frm = $f; break } } }; if($frm -and $frm.Tag.p){ Exec-File 'open' $frm.Tag.p } })
    $btnOpenWith.Add_Click({ $frm = $null; try { $frm = $this.FindForm() } catch {}; if (-not $frm) { foreach ($f in [System.Windows.Forms.Application]::OpenForms) { if ($f.Controls["pnlSettings"]) { $frm = $f; break } } }; if($frm -and $frm.Tag.p){ Exec-File 'with' $frm.Tag.p } })
    ...
    $cmbReaderStr.Add_TextChanged({ $frm = $null; try { $frm = $this.FindForm() } catch {}; if (-not $frm) { foreach ($f in [System.Windows.Forms.Application]::OpenForms) { if ($f.Controls["pnlSettings"]) { $frm = $f; break } } }; if($frm -and -not $frm.Tag.initializing){$timer=$frm.Tag.searchTimer; $timer.Tag=$frm; $timer.Stop(); $timer.Start()} })
    $cmbReaderStr.Add_KeyDown({ if($_.KeyCode -eq "Enter") { $frm = $null; try { $frm = $this.FindForm() } catch {}; if (-not $frm) { foreach ($f in [System.Windows.Forms.Application]::OpenForms) { if ($f.Controls["pnlSettings"]) { $frm = $f; break } } }; if($frm){$frm.Tag.searchTimer.Stop(); $frm.Tag.applySearch.Invoke($frm, $true)}; $_.SuppressKeyPress=$true } })
    $chkWholeWordReader.Add_CheckedChanged({ $frm = $null; try { $frm = $this.FindForm() } catch {}; if (-not $frm) { foreach ($f in [System.Windows.Forms.Application]::OpenForms) { if ($f.Controls["pnlSettings"]) { $frm = $f; break } } }; if($frm -and -not $frm.Tag.initializing){$frm.Tag.searchTimer.Stop(); $frm.Tag.applySearch.Invoke($frm, $false)} })
    $chkRegexReader.Add_CheckedChanged({ $frm = $null; try { $frm = $this.FindForm() } catch {}; if (-not $frm) { foreach ($f in [System.Windows.Forms.Application]::OpenForms) { if ($f.Controls["pnlSettings"]) { $frm = $f; break } } }; if($frm -and -not $frm.Tag.initializing){$frm.Tag.searchTimer.Stop(); $frm.Tag.applySearch.Invoke($frm, $false)} })
