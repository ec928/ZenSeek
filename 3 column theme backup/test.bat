...
$script:themes = Get-Content $themesFile -Raw | ConvertFrom-Json
...
function Show-Reader($directFilePath = $null) {
    ...
    $btnTheme.Add_Click({
        $cmXaml = @"
<ContextMenu xmlns='http://schemas.microsoft.com/winfx/2006/xaml/presentation' xmlns:x='http://schemas.microsoft.com/winfx/2006/xaml' Background='#1A1A1A' Foreground='#E0E0E0' BorderBrush='#333333' BorderThickness='1' Padding='4'>
            ...
            <Setter.Value>
                <ControlTemplate TargetType='MenuItem'>
                    <Border Name='Border' Background='Transparent' CornerRadius='2'>
                        <Grid>
                            <Path x:Name='CheckMark' Visibility='Collapsed' Data='M 0,5 L 3,8 L 9,0' Stroke='White' StrokeThickness='2' Margin='10,0,0,0' VerticalAlignment='Center' HorizontalAlignment='Left'/>
  ...
  <StackPanel Orientation='Horizontal'>
    <StackPanel x:Name='colDark' Margin='0,0,16,0'><TextBlock Text='Dark Themes' Foreground='#888888' FontWeight='SemiBold' Margin='12,4,8,8' FontSize='11'/></StackPanel>
    <StackPanel x:Name='colLight' Margin='0,0,16,0'><TextBlock Text='Light Themes' Foreground='#888888' FontWeight='SemiBold' Margin='12,4,8,8' FontSize='11'/></StackPanel>
    <StackPanel x:Name='colMono'><TextBlock Text='Mono Themes' Foreground='#888888' FontWeight='SemiBold' Margin='12,4,8,8' FontSize='11'/></StackPanel>
  </StackPanel>
</ContextMenu>
"@
        $cm = [Windows.Markup.XamlReader]::Parse($cmXaml)
        ...
        for ($i = 0; $i -lt $script:themes.Count; $i++) {
            ...
            $mi.Header = $t.Name
            $mi.Tag = $i
            if ($i -eq $this.FindForm().Controls["pnlSettings"].Controls["cmbTheme"].SelectedIndex) { $mi.IsChecked = $true }
            
            $mi.Add_Click({
                $idx = $this.Tag
                $frm = $this.FindForm()
                $frm.Controls["pnlSettings"].Controls["cmbTheme"].SelectedIndex = $idx
                $this.Text = $script:themes[$idx].Name
            })
        ...
        $darkList | Sort-Object Header | ForEach-Object { $colDark.Children.Add($_) }
        $lightList | Sort-Object Header | ForEach-Object { $colLight.Children.Add($_) }
        $monoList | Sort-Object Header | ForEach-Object { $colMono.Children.Add($_) }
...
'@
    ...
    $cmbTheme.Add_SelectedIndexChanged({ 
        ...
        if ($frm -and -not $frm.Tag.initializing) { 
            ...
            if ($idx -ge 0 -and $idx -lt $script:themes.Count) {
                ...
                if ($cF.Items.Contains($t.FN)) { $cF.SelectedItem = $t.FN } else { $cF.SelectedItem = "Consolas" }
    ...
    $btnReset.Add_Click({ 
        ...
        if ($frm -and -not $frm.Tag.initializing) { 
            ...
            if (Test-Path $tFile) {
                $factory = Get-Content $tFile -Raw | ConvertFrom-Json
                if ($idx -ge 0 -and $idx -lt $factory.Count) {
                    ...
                    $cF = $pSet.Controls["cmbFont"]
                    if ($cF.Items.Contains($script:themes[$idx].FN)) { $cF.SelectedItem = $script:themes[$idx].FN } else { $cF.SelectedItem = "Consolas" } 
    ...
    $t = $script:themes[$script:activeTheme]
    $cmbFont.SelectedItem = "Consolas"
    if ($cmbFont.Items.Contains($t.FN)) { $cmbFont.SelectedItem = $t.FN }
    $numFont.Value = $t.FS 
